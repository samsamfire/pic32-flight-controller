...
#define BMP280_Start    I2C5_Start
#define BMP280_Write    I2C5_Write
#define BMP280_Read     I2C5_Read
#define BMP280_Stop     I2C5_Stop
...
static int32_t adc_T, adc_P, t_fine;
                      ...
                      BMP280_sampling P_sampling, BMP280_filter filter, standby_time standby)
{
  ...
  writeByte(BMP280_I2C_ADDRESS,BMP280_REG_CONFIG,  _config);
  writeByte(BMP280_I2C_ADDRESS,BMP280_REG_CONTROL, _ctrl_meas);
}
...
{
  if(readByte(BMP280_I2C_ADDRESS,BMP280_REG_CHIPID) != BMP280_CHIP_ID)
    return 0;

  // reset the BMP280 with soft reset
  writeByte(BMP280_I2C_ADDRESS,BMP280_REG_SOFTRESET, 0xB6);
  delay_ms(100);

  // if NVM data are being copied to image registers, wait 100 ms
  while ( (readByte(BMP280_I2C_ADDRESS,BMP280_REG_STATUS) & 0x01) == 0x01 )
    delay_ms(100);
...
{
  uint8_t ctrl_meas_reg = readByte(BMP280_I2C_ADDRESS,BMP280_REG_CONTROL);

  if ( (ctrl_meas_reg & 0x03) != 0x00 )
    return 0;   // sensor is not in sleep mode

  // set sensor to forced mode
  writeByte(BMP280_I2C_ADDRESS,BMP280_REG_CONTROL, ctrl_meas_reg | 1);
  // wait for conversion complete
  while (readByte(BMP280_I2C_ADDRESS,BMP280_REG_STATUS) & 0x08)
    delay_ms(1);
